...
int main(int argc, char * argv[])
{
	...
	// Сохранение отсортированного
#ifndef _MSC_VER
	fpOUTtext = fopen("outputSort.txt", "wb");
	fpOUTbin = fopen("outputSort.bin", "wb");
#else
	fpOUTtext = NULL;
	fpOUTbin = NULL;
	fopen_s(&fpOUTtext, "outputSort.txt", "wb");
	fopen_s(&fpOUTbin, "outputSort.bin", "wb");
#endif
	if (fpOUTtext != NULL) fprintf(fpOUTtext, "t");
	else printf("error to work with outputSort.txt\n");
	if (fpOUTbin != NULL) fprintf(fpOUTbin, "b");
	else printf("error to work with outputSort.bin\n");
	if (fpOUTtext != NULL)
	{
		if (Purchase_Array_fPrintln(fpOUTtext, arr) != arr.length)
			printf("error to save outputSort.txt");
		fclose(fpOUTtext);
	}
	else printf("error of write to outputSort.txt\n");
	if (fpOUTbin != NULL)
	{
		if (Purchase_Array_fwrite(fpOUTbin, arr) != arr.length)
			printf("error to save outputSort.bin");
		fclose(fpOUTbin);
	}
