...
// const char * message: указатель на начало строки сообщения.
void z7_16_test_UserInterface_p_log(char res, const char * message)
{
...
void z7_16_test_UserInterface_p_logStringFloat(char res, const char * message, float value)
{
	if (res == 0) printf("\tError: ");
	printf("%s%f\n", message, value);
}
...
void z7_16_test_UserInterface_p_logStringUnsignedInt(char res, const char * message, unsigned value)
{
	if (res == 0) printf("\tError: ");
	printf("%s%u\n", message, value);
}
